...
void test_tpm_counter() {
    sgxbutil::MonoCounterManager& manager = sgxbutil::GetGlobalMonoCntManager();
    sgxbutil::CounterVal counter_value = 0;
    sgxbutil::CounterID counter_id;
    counter_id = manager.get_counter();
    VLOG(79) << "Func: " << __FUNCTION__ << " counter_id = " << counter_id;
    counter_value = manager.read_counter(counter_id);
    VLOG(79) << "Func: " << __FUNCTION__ << " counter_value = " << counter_value;
    manager.increase_counter(counter_id);
    counter_value = manager.read_counter(counter_id);
    VLOG(79) << "Func: " << __FUNCTION__ << " counter_value (increased) = " << counter_value;
}
